...
class SinglePostView(View):
  ...
  def post(self, request, slug):
      ...
      post = Post.objects.get(slug=slug)
      # if form is valid, take the user input, add extra data and save that edited data back to the database
      if comment_form.is_valid():
        # With commit=False, calling save will not hit the database, but will create a new model instance
        comment = comment_form.save(commit=False)
        comment.post = post
        comment.save()
      # Reverse function allows us to construct urls automatically by just jusing the names of urls
